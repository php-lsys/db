 ...
 */
namespace LSYS\Database\RWCache\Parse;
use LSYS\Database\RWCache\Parse;
use LSYS\Database;
class Simple implements Parse {
    private $_db_name;
    public function __construct($default_db_name=null){
        $this->_db_name=$default_db_name;
    }
    private function _table($table){
        $table=trim($table,'`"');
        if($this->_db_name&&strpos($table, '.')===false)$table=$this->_db_name.'.'.$table;
        return $table;
    }
    /**
     * @return array $table
